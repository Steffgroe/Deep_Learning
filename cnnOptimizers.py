...
def generate_model(dropout,HIDDEN_UNITS,activation,optimizer,lr,x_train):
	if optimizer is 'sgd':
		optimizer = tensorflow.keras.optimizers.SGD(lr=lr ,decay=1e-6, momentum=0.9, nesterov=True)
	if optimizer is 'adadelta':
		optimizer = tensorflow.keras.optimizers.Adadelta(learning_rate=lr, rho=0.95)
	else:
		optimizer = 'adam'
...
for idx in range (0,len(optimizers)):
	for idx2 in range(0,4):
		model = generate_model(0.2,512,'leaky',optimizers[idx],learning_rate,x_train)
